...
export default function getUmdIntro ( imports, exports, options ) {
	var template,
		intro,
		importPaths = [],
		importNames = [],
	...
	imports.forEach( function ( x, i ) {
		...
		if ( !x.specifiers.length && ( i === imports.length - 1 ) ) {
			// don't include
		} else {
			importNames[i] = name;
		}
	...
	if ( options.defaultOnly || !exports.length ) {
